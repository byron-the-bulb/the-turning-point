...
async def greeting_ready_handler(args: FlowArgs) -> FlowResult:
    ...
    logger.info(f"[Flow]greeting_ready_handler: {user_ready} -> {is_ready}")
...
async def greeting_callback(
    ...
    if result["user_ready"]:
        logger.info("[Flow]User is ready, moving to collect_name")
        await flow_manager.set_node("collect_name", create_collect_name_node())
    else:
        logger.info("[Flow]User is not ready, staying in greeting")
...
async def collect_name_callback(
    ...
    if result["status"] == "success" and result.get("user_name"):
        logger.info(f"[Flow]Name collected: {result['user_name']}, moving to consider_challenge")
        await flow_manager.set_node("consider_challenge", create_consider_challenge_node())
    else:
        logger.info("[Flow]No name collected, staying in collect_name")
        await flow_manager.set_node("collect_name", create_collect_name_node())

def create_collect_name_node()->NodeConfig:
    return {
        "role_messages": [
            {"role": "system", "content": "You are Sphinx, a therapeutic guide. Your task is to collect the participant's name."}
        ],
        "task_messages": [
...
async def consider_challenge_handler(args: FlowArgs) -> FlowResult:
    user_ready = args.get("user_ready", "").lower()
    is_ready = any(word in user_ready for word in ["ready", "proceed", "continue", "yes", "yeah"])
    logger.info(f"[Flow]consider_challenge_handler: {user_ready} -> {is_ready}")
    return {"status": "success", "user_ready": is_ready}
...
async def consider_challenge_callback(
    ...
    if result["user_ready"]:
        logger.info("[Flow]User is ready, moving to select_challenge")
        await flow_manager.set_node("select_challenge", create_select_challenge_node())
    else:
        logger.info("[Flow]User is not ready, staying in consider_challenge")
        await flow_manager.set_node("consider_challenge", create_consider_challenge_node())

def create_consider_challenge_node()->NodeConfig:
    return {
        "role_messages": [
            {"role": "system", "content": "You are Sphinx, a therapeutic guide. Your task is to help the participant identify their challenge."}
        ],
        "task_messages": [
