...
class MCPClientCustom(BaseObject):
    def __init__(
        self,
        ...
        async def mcp_tool_wrapper(
            ...
            try:
                async with self._client(self._server_params) as (read, write):
                    # Reuse (or create) a persistent session
    ...
    async def initialize(self):
        """Public method to explicitly initialize the MCP session.

        Mirrors the `Server` example in the official SDK: creates the streams
        context, enters it, and calls `ClientSession.initialize()` so that the
        caller is certain the session is ready for use.
        Returns the active ``ClientSession`` instance so that external callers
        (e.g. a Flow manager) can interact with it if needed.
        """
        return await self._get_or_create_session()

    async def cleanup(self):
        """Public method to gracefully tear-down the MCP session."""
        logger.debug("Cleaning up MCP session")
        if self.session is None:
            logger.debug("MCP session is already closed")
            return
        async with self._cleanup_lock:
            try:
                ...
                logger.warning(f"Error while closing MCP session: {e}")
            

    async def _get_or_create_session(self):
        """Return an active ``ClientSession``; create it lazily if necessary.

        Uses an ``asyncio.Lock`` to ensure only one coroutine performs the
        expensive creation/initialization sequence, mirroring the behaviour of
        the SDK's ``Server`` implementation.
        """
        logger.debug("Getting or creating MCP session")

        # Fast-path: if a live session already exists return it immediately.
        if self.session is not None and getattr(self.session, "closed", False) is False:
            return self.session

        transport = await self.exit_stack.enter_async_context(self._client(self._server_params))
